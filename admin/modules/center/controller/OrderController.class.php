 ...
 * @since  2015-01-23
 */
class OrderController extends BcController {
    ...
    public function defaultAction() {
        ...
        try {
            ...
            $content = $this->view->fetch('order/index.php', $data);
            echo $content;
        } catch (Exception $e) {
    ...
    public function matchAction() {
        ...
        $content = $this->view->fetch('order/match.php', $data);
        echo $content;
    }
    
    /**
     * @brief 匹配育儿嫂弹层确认育儿嫂信息
     */
    public function iframeMatchAction() {
        try {
            ...
            if (empty($info)) {
                throw new Exception("ID为{$id}的育儿嫂可能还未签约、获取其信息失败！");
            }
            $faceParams = array(
            ...
            $content = $this->view->fetch('order/iframe_show_parental.php', $data);
        } catch (Exception $e) {
    ...
    public function subMatchAction() {
        ...
        if (empty($id) || empty($orderId)) {
            echo json_encode(array('errorCode' => 1, 'msg' => '育儿嫂的ID或者订单ID不正确，请仔细核对！'));
        }
    ...
    public function auditionAction() {
        ...
        $content = $this->view->fetch('order/audition.php', $data);
        echo $content;
    }
    ...
    public function ajaxSubmitAuditionAction() {
        ...
        if (empty($auditionAddress) || empty($auditionTime) || empty($auditionHour)) {
    ...
    public function contractAction() {
        ...
        try {
            ...
            $content = $this->view->fetch('order/contract.php', $data);
            echo $content;
        } catch (Exception $e) {
            $this->errorAction(array($e->getMessage()));
        }
    ...
    public function ajaxAddParentalOrderAction() {
        ...
        if (empty($hourB) || empty($hourE)) {
            exit(json_encode(array('errorCode' => 1, 'msg' => '服务开始和结束的小时必须选择')));
        }
        ...
        $info = array(
            ...
            'service_time_b'  => !empty($serviceTimeB) ? strtotime($serviceTimeB) + 3600 * $hourB : 0,
            'service_time_e'  => !empty($serviceTimeE) ? strtotime($serviceTimeE) + 3600 * $hourE : 0,
        );
    ...
    /**
     * @brief 获取订单关联的匹配的育儿嫂信息、（待面试、或者面试中得）
     * @param $orderId 订单ID
     */
        ...
        try {
            ...
            if (!empty($info) && is_array($info)) {
                ...
                foreach ($info as $i) {
                    ...
                    //收集经济人id
                    !empty($i['manager_id']) && $managerIds[] = $i['manager_id'];
                }
    ...
    private function _setField($data = array()) {
        ...
        if (!empty($data['service_time_e']) && $data['service_time_e'] > 0) {
            $endDate = date("Y-m-d", $data['service_time_e']);
            $endHour = (int) date("H", $data['service_time_e']);
        }
        //服务结束日期
        $serviceEField = array(
            'name'      => 'service_time_e',
            'tipTarget' => '#service_time_e_tip',
            'next'      => 'hour_e',
            'postValue' => !empty($endDate) ? $endDate : 'null',
            'rules' => array(
                array(
                    'mode' => RuleConfig::REQUIRED,
                    'errorMessage' => '服务结束日期必须填写',
                ),
            ),
        );
        ...
        $hourEField = array(
            'name'      => 'hour_e',
            'tipTarget' => '#service_time_e_tip',
            'defaultMessage' => '填写日期、同时选择具体结束的小时数',
            'postValue' => !empty($endHour) ? $endHour : 'null',
            'rules' => array(
                array(
                    'mode' => RuleConfig::REQUIRED,
                    'errorMessage' => '服务结束时间必须选择',
                ),
            ),
        );
