...
class OrderYuesaoListController extends BcController {
    ...
    public function defaultAction() {
        $data = array(
            ...
            'statusMap' => BcEnumerateConfig::$ORDER_YUESAO_STATUS,
        );
        $content = $this->view->fetch('order_yuesao_list/index.php', $data);
        echo $content;
    }
    ...
    public function getDataGrid() {
        ...
        $dataGrid->setUrl('/center/orderYuesaoList/ajaxGetData/?' . $this->getQueryString());
        $dataGrid->setCol('field1', '订单编号', 'width:10%');
        $dataGrid->setCol('field2', '订单联系人', 'width: 10%');
        $dataGrid->setCol('field3', '月嫂姓名', 'width:10%');
    ...
    public function ajaxGetDataAction() {
        ...
        if (!empty($info) && is_array($info)) {
            ...
            foreach ($info as $order) {
                ...
                $row['field4'] = isset(BcEnumerateConfig::$ORDER_YUESAO_STATUS[$order['status']]) ?
                    BcEnumerateConfig::$ORDER_YUESAO_STATUS[$order['status']] : '-';
                $row['field5'] = $serviceTimeB . '<br/>' . $serviceTimeE;
                $row['field6'] = '<a target="_blank" href="/center/orderYuesaoList/detail/?id=' . $id . '">查看</a>';
                if (in_array('bc.*', $code) || in_array('bc.service', $code)) {
                    $row['field6'] .= ' | <a target="_blank" href="/center/orderYuesao/default/?id=' . $id . '">修改订单</a>';
                }
                if (in_array('bc.*', $code) || in_array('bc.service', $code) || in_array('bc.manager', $code)) {
                    if ($order['status'] == BcEnumerateConfig::YUESAO_ORDER_STATUS_SERVE) {
                        $row['field6'] .= ' | <a target="_blank" href="/center/orderCommon/log/?order_id=' . $id
                            . '&order_type='.GlobalConfig::ORDER_TYPE_MONTH.'">日志追踪</a>';
                    }
                }
                $list[] = $row;
            }
        ...
        try {
            ...
            if (!empty($info) && is_array($info)) {
                ...
                $orderInfo = array(
                    ...
                    'status_text'     => isset(BcEnumerateConfig::$ORDER_YUESAO_STATUS[$info['status']])
                                         ? BcEnumerateConfig::$ORDER_YUESAO_STATUS[$info['status']] : '',
                    ...
                    foreach ($ops as $k => $op) {
                        switch ($op) {
                            case 1: //匹配育儿嫂
                                ...
                                if (in_array('bc.*', $code) || in_array('bc.service', $code) || in_array('bc.manager', $code)) {
                                    $opUrl .= '<a target="_blank" href="/center/orderYuesao/match/?order_id=' . $orderId . '" class="btn btn-success">匹配月嫂</a>';
                                }
                                break;
                            case 2: //预约面试
                                ...
                                if (in_array('bc.*', $code) || in_array('bc.service', $code)) {
                                    $opUrl .= '<a target="_blank" href="/center/orderYuesao/audition/?order_id=' . $orderId . '" class="btn btn-success">预约面试</a>';
                                }
                                break;
                            case 3: //结算订单
                                ...
                                if (count($subInfo) > 0 && (in_array('bc.*', $code) || in_array('bc.service', $code))) {
                                    $opUrl .= '<a target="_blank" href="/center/orderYuesao/over/?order_id=' . $orderId . '" class="btn btn-success">结算订单</a>';
                                }
                                break;
                            case 4: //签合同&余款
                                if ((in_array('bc.*', $code) || in_array('bc.service', $code)) && !empty($info['worker_id'])) {
                                    $opUrl .= '<a target="_blank" href="/center/orderYuesao/contract/?order_id=' . $orderId . '" class="btn btn-success">签合同 & 余款</a>';
                                }
                                break;
                            case 5: //续约订单
                                if (in_array('bc.*', $code) || in_array('bc.service', $code)) {
            ...
            if ($orderInfo['status'] == OrderConfig::STATUS_SERVE || $orderInfo['status'] == OrderConfig::STATUS_OVER) {
                ...
                $this->view->assign('comCat', SelfConfig::$YUESAO_COMMENT_CAT);//月嫂点评评分类别
                //获取回访列表数据
                $visitClient = Rpc::client('order.visit');
                $visitParams = array(
                    'filters' => array(
                        array('order_id', '=', $orderId),
                        array('order_type', '=', GlobalConfig::ORDER_TYPE_MONTH),
                    ),
                );
            ...
            $content = $this->view->fetch('order_yuesao_list/detail.php', $data);
            echo $content;
    ...
    public function iframeStopAction() {
        ...
        $content = $this->view->fetch('order_yuesao_list/iframe_stop.php', $data);
        echo $content;
    }
    ...
    public function iframeScheduleAction() {
        ...
        $data = array('orderId' => $orderId);
        $content = $this->view->fetch('order_yuesao_list/iframe_chg_schedule.php', $data);
        echo $content;
    }
