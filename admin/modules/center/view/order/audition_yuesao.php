...
G.use(['app/backend/js/backend.js', 'jquery'], function (Backend, $) {
    ...
    Backend.on('sub_result', function (e) {
        ...
        $.ajax({
            url : "/center/orderYuesao/ajaxSubAuditionResult/",
            type : "POST",
            dataType : "json",
            data : {
            ...
            success : function(result) {
                if (result.errorCode) {
                    ...
                    window.setTimeout(function() {
                        if (result.is_pass) {
                            location.href="/center/orderYuesaoList/detail/?id=<?php echo $_GET['order_id'];?>";
                        } else {
                            location.reload();
                        }
                    }, 2000);
    ...
    Backend.on('submit', function (e) {
        ...
        $this.addClass('disabled');
        $.ajax({
            url : "/center/orderYuesao/ajaxSubmitAudition/",
            type : "POST",
            dataType : "json",
            data : {
