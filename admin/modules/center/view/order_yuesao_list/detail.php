...
<div class="wrapper row-offcanvas row-offcanvas-left">
    ...
    <aside class="right-side">
        ...
        <div class="callout callout-info">
            ...
            <input type="hidden" name="order_type" value="<?php echo GlobalConfig::ORDER_TYPE_MONTH;?>"/>
                            ...
                            <tr>
                                <td colspan="2" style="padding: 10px;">
                                    <?php foreach ($this->comCat as $key => $cat) {?>
                                        <?php echo $cat;?>：<?php echo $com['yuesao_'.$key];?>分&nbsp;&nbsp;
                                    <?php }?>
                                </td>
                            </tr>
                        </table>
                    ...
                    <?php if (!empty($this->comCat) && is_array($this->comCat)) {?>
                        <?php foreach ($this->comCat as $key => $cat) {?>
                            <tr>
                                ...
                                <td style="padding: 6px;"><input type="radio" name="yuesao_<?php echo $key;?>" value="1"/> 1 </td>
                                <td style="padding: 6px;"><input type="radio" name="yuesao_<?php echo $key;?>" value="2"/> 2 </td>
                                <td style="padding: 6px;"><input type="radio" name="yuesao_<?php echo $key;?>" value="3"/> 3 </td>
                                <td style="padding: 6px;"><input type="radio" name="yuesao_<?php echo $key;?>" value="4"/> 4 </td>
                                <td style="padding: 6px;"><input type="radio" name="yuesao_<?php echo $key;?>" value="5"/> 5 </td>
                            </tr>
                        <?php }?>
                    <?php }?>
                </table>
...
G.use(['app/backend/js/backend.js', 'jquery', 'widget/dialog/dialog.js'], function (Backend, $, Dialog) {
    ...
    Backend.on('stop_order', function (e) {
        e.preventDefault();
        var id = $(this).data('id');
        var url = "/center/orderYuesaoList/iframeStop/?id="+id;
    ...
    Backend.on('chg_schedule', function (e) {
        e.preventDefault();
        var id = $(this).data('id');
        var url = "/center/orderYuesaoList/iframeSchedule/?id="+id;
        var dialog = new Dialog({
            title : '填写新的档期的起止时间',
            width : 550,
            height: 350,
    ...
    Backend.on('sub_comment', function (e) {
        ...
        $.ajax({
            ...
            data : {
                ...
                "yuesao_1" : $('input:radio[name="yuesao_1"]:checked').val(),
                "yuesao_2" : $('input:radio[name="yuesao_2"]:checked').val(),
                "yuesao_3" : $('input:radio[name="yuesao_3"]:checked').val(),
                "yuesao_4" : $('input:radio[name="yuesao_4"]:checked').val(),
                "yuesao_5" : $('input:radio[name="yuesao_5"]:checked').val()
