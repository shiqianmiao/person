...
<div id="head">
	...
	<div id="main">
    	<div id="improve_safe1">
        	<div class="step3 s3_1">
            	<ol class="clearfix">
                	<li>重置密码</li>
                	<li>绑定手机号</li>
                	<li>完成</li>
            ...
            <form method="post" class="form1" name="resetPwdForm" action="/ImproveSave/resetPwd/" autocomplete="off">
                <ul class="u1">
                    <li class="clearfix">
                    	<label>现在的密码：</label><input type="password" class="input1" name="oldPasswd" /><div class="msg m4"><i class="i1"></i>请输入现在的密码</div>
                    </li>
                	<li class="clearfix">
                    	<label>输入新密码：</label><input type="password" class="input1" name="newPasswd" /><div class="msg m4" style="display:none;"><i class="i1"></i>密码必须包含字母和数字的组合，长度8～16位，字母区分大小写</div>
                    </li>
                	<li class="clearfix">
                    	<label>再次输入新密码：</label><input type="password" class="input1" name="notPasswd" />
                    </li>
                </ul>
                <input type="hidden" name="type" value="<?php echo RequestUtil::getGET('type', ''); ?>" />
                <div class="btn1 clearfix"><input type="submit" class="button1" name="submit" value="下一步" />
                <?php echo $this->fetch('common/result_msg.php');
                if (!$this->resultMsg) { ?>
                    <span>下一步将进行手机号绑定</span></div>
                <?php }?>
            </form>
        </div>
    </div>
    ...
    G.use(['jquery', 'app/v3/js/profile/profile.js'], function($, FormValidator){
        ...
        var $resetPwdForm = $('form[name=resetPwdForm]');
         if ($resetPwdForm.length == 1) {
             ...
             a.addCheckProject('notPasswd', 'equal', '两次密码输入不一致', {isStop:false, compareNum:'newPasswd', bind:'blur', showCorrect:true});
             a.addCheckProject(['oldPasswd'], 'ajax', ['密码输入有误,请重新输入'], {url:'/improveSave/ajaxcheckpwd/', preTrue:true});
             a.checkSubmit();
         }
    });
</script>
